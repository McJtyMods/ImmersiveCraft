...
public class InWorldVerticalPlacerTE extends GenericInventoryTE {
    ...
    public InWorldVerticalPlacerTE() {
        super(4);
        addInterfaceHandle(new InputInterfaceHandle().slot(SLOT_INPUT1).side(EnumFacing.SOUTH).bounds(0, .5f, .5f, 1).renderOffset(new Vec3(-.23, 0.23, .23)));
        addInterfaceHandle(new InputInterfaceHandle().slot(SLOT_INPUT2).side(EnumFacing.SOUTH).bounds(.5f, .5f, 1, 1).renderOffset(new Vec3(.23, 0.23, .23)));
        addInterfaceHandle(new InputInterfaceHandle().slot(SLOT_INPUT3).side(EnumFacing.SOUTH).bounds(0, 0, .5f, .5f).renderOffset(new Vec3(-.23, -0.23, .23)));
        addInterfaceHandle(new InputInterfaceHandle().slot(SLOT_INPUT4).side(EnumFacing.SOUTH).bounds(.5f, 0, 1, .5f).renderOffset(new Vec3(.23, -0.23, .23)));
    }
